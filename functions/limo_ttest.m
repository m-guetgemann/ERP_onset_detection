function [m,dfe,ci,sd,n,t,p, tcrit] = limo_ttest(type,data1,data2,alphav)

% [m,dfe,ci,sd,n,t,p,tcrit] = limo_ttest(type,data1,data2,alpha)
...
% OUTPUTS:
%
% [m,dfe,ci,sd,n,t,p, tcrit] = means, dfe, confidence interavals, std, 
%                   number of observations, t values, p values, critical
%                   two-tailed threshold
%
% ONLY NEW adapted version outputs tcrit, the critical two-tailed threshold
%
...
switch (type)
    ...
    case(1)
        ...
        c = tinv((1 - alphav / 2), n - 1) .* (sd ./ sqrt(n));
         ...
         % alteration; output critical two-tailed threshold
        tcrit = tinv(1 - alphav/2, n - 1);   % scalar
    ...
    case(2)
        ...
        % alteration; output critical two-tailed threshold
        tcrit = tinv(1 - alphav/2, dfe);   % vector (size of time points)
